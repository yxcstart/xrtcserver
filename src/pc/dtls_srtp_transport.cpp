...
void DtlsSrtpTransport::_setup_dtls_srtp() {
    ...
    if (!_extract_params(_rtp_dtls_transport, &selected_crypto_suite, &send_key, &recv_key) ||
        !set_rtp_params(selected_crypto_suite, &send_key[0], send_key.size(), send_extension_ids, selected_crypto_suite,
                        &recv_key[0], recv_key.size(), recv_extension_ids)) {
        RTC_LOG(LS_WARNING) << "DTLS-SRTP rtp param install failed";
    }
}

bool DtlsSrtpTransport::_extract_params(DtlsTransport* dtls_transport, int* selected_crypto_suite,
                                        rtc::ZeroOnFreeBuffer<unsigned char>* send_key,
                                        rtc::ZeroOnFreeBuffer<unsigned char>* recv_key) {
    ...
    if (!dtls_transport->export_keying_material(k_dtls_srtp_exporter_label, nullptr, 0, false, &dtls_buffer[0],
                                                dtls_buffer.size())) {
...
int DtlsSrtpTransport::send_rtp(const char* buf, size_t size) {
    ...
    if (!protect_rtp(data, len, packet.capacity(), &len)) {
        RTC_LOG(LS_WARNING) << "Failed to protect rtp packet, size=" << len << ", seqnum=" << seq_num
                            << ", ssrc=" << parse_rtp_ssrc(packet) << ", last_send_seq_num=" << _last_send_seq_num;
        return -1;
    }
...
int DtlsSrtpTransport::send_rtcp(const char* buf, size_t size) {
    ...
    if (!protect_rtcp(data, len, packet.capacity(), &len)) {
        int type = 0;
        get_rtcp_type(data, len, &type);
        RTC_LOG(LS_WARNING) << "Failed to protect rtcp packet, size=" << len << ", type=" << type;
        return -1;
    }
