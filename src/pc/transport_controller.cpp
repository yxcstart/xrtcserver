...
int TransportController::set_local_description(SessionDescription* desc) {
    ...
    for (auto content : desc->contents()) {
        ...
        dtls->signal_writable_state.connect(this, &TransportController::_on_dtls_wriatble_state);
...
void TransportController::_on_dtls_wriatble_state(DtlsTransport*) { _update_state(); }

void TransportController::_on_dtls_state(DtlsTransport*, DtlsTransportState) { _update_state(); }

void TransportController::_on_ice_state(IceAgent*, IceTransportState) { _update_state(); }
...
void TransportController::_update_state() {
    ...
    int total_connected =
        dtls_state_counts[DtlsTransportState::k_connected] + ice_state_counts[IceTransportState::k_connected];
...
int TransportController::set_remote_description(SessionDescription* desc) {
    ...
    for (auto content : desc->contents()) {
        ...
        if (td) {
            ...
            if (dtls) {
                dtls->set_remote_fingerprint(td->identify_fingerprint->algorithm,
                                             td->identify_fingerprint->digest.cdata(),
                                             td->identify_fingerprint->digest.size());
            }
        }
    }
