...
bool has_correct_rtp_version(rtc::ArrayView<const uint8_t> packet) { return packet[0] >> 6 == k_rtp_version; }

bool payload_type_is_reserved_for_rtcp(uint8_t payload_type) { return 64 <= payload_type && payload_type < 96; }

bool is_rtp_packet(rtc::ArrayView<const uint8_t> packet) {
    return packet.size() >= k_min_rtp_packet && has_correct_rtp_version(packet) &&
           !payload_type_is_reserved_for_rtcp(packet[1] & 0x7F);
}

bool is_rtcp_packet(rtc::ArrayView<const uint8_t> packet) {
    return packet.size() >= k_min_rtcp_packet && has_correct_rtp_version(packet) &&
           payload_type_is_reserved_for_rtcp(packet[1] & 0x7F);
}
...
bool get_rtcp_type(const void* data, size_t len, int* type) {
    if (len < k_min_rtcp_packet) {
        return false;
    }
    if (!data || !type) {
        return false;
    }
    *type = *(static_cast<const uint8_t*>(data) + 1);
    return true;
}
