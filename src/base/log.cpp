...
XrtcLog::XrtcLog(const std::string &log_dir, const std::string &log_name,
    ...
    : _log_dir(log_dir),
      ...
      _log_file_wf(_log_dir + "/" + log_name + ".log.wf") {}
XrtcLog::~XrtcLog() {
  stop();
  _out_file.close();
  _out_file_wf.close();
...
bool XrtcLog::start() {
  if (_running) {
    fprintf(stderr, "log thread has already running");
    return false;
  }
...
void XrtcLog::set_log_to_stderr(bool on) { rtc::LogMessage::SetLogToStderr(on); }
}  // namespace xrtc